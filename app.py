...
def api_stop():
    """Signale au thread de session qu'il doit s'arreter."""
    global _session_should_stop
    _session_should_stop = True
    _emit({"type": "info", "message": "Arret demande par l utilisateur"})
    _emit({"type": "done", "message": "Session arretee", "count": 0})
...
def _run_session(config: dict):
    global _session_running, _last_results, _last_config, _last_xlsx, _session_should_stop
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .run-buttons-row { display:flex; gap:10px; margin-top:24px; align-items:stretch; }
  .run-buttons-row .btn-run { flex:1; margin-top:0; }
  .btn-ctrl {
    width:58px; padding:0; font-size:20px;
    background:transparent; border:1px solid var(--border);
    color:var(--muted); border-radius:10px; cursor:pointer;
    transition:all .2s; font-family:var(--sans);
    display:flex; align-items:center; justify-content:center;
  }
  .btn-ctrl:not(:disabled):hover { border-color:var(--amber); color:var(--amber); background:rgba(245,166,35,.06); }
  .btn-ctrl:disabled { opacity:.35; cursor:not-allowed; }
  .btn-ctrl.paused { border-color:var(--amber); color:var(--amber); background:rgba(245,166,35,.1); }
  .btn-ctrl-stop:not(:disabled):hover { border-color:var(--red); color:var(--red); background:rgba(239,68,68,.06); }
...
<div class="wrap">
  ...
  <div class="panel" id="config-panel">
    ...
    <!-- 1. SOURCES DE DONNÉES (en premier, applique à tous) -->
    <div class="field">
      <label>Sources de données <span class="field-hint">Sélectionne 1 ou plusieurs</span></label>
      <small class="field-desc">Ces sources sont utilisées pour TOUTES les recherches ci-dessous (niche et mots-clés).</small>
    ...
    <!-- 2. NICHE / SOUS-NICHE / FENÊTRE / MARCHÉS -->
    <div class="form-grid">
      <div class="field">
        <label>Niche</label>
        <select id="niche-select" onchange="onNicheChange()">
          ...
          <option value="Ecology & Sustainable">Ecology & Sustainable</option>
          ...
          <option value="Luxury & Premium Lifestyle">Luxury & Premium Lifestyle</option>
      ...
      <div class="field">
        ...
        <div class="checkbox-group" id="markets">
          <button type="button" class="chip active" data-val="USA">🇺🇸 USA</button>
          <button type="button" class="chip active" data-val="Canada">🇨🇦 Canada</button>
          <button type="button" class="chip" data-val="France">🇫🇷 France</button>
          <button type="button" class="chip" data-val="UK">🇬🇧 UK</button>
          <button type="button" class="chip" data-val="Germany">🇩🇪 Germany</button>
          <button type="button" class="chip" data-val="Europe">🇪🇺 Europe</button>
        </div>
      </div>
    </div>
    ...
    <!-- 3. MOTS-CLÉS PRODUITS (unifié) -->
    <div class="field">
      <label>Mots-clés produits <span class="field-hint" id="keywords-badge">Aucune source</span></label>
      <small class="field-desc">Mots-clés appliqués aux sources sélectionnées ci-dessus. Sépare par des virgules. Laisse vide pour scanner toute la niche.</small>
      <div class="kw-input-wrap">
        <input type="text" id="keywords" placeholder="ex : posture corrector, back pain relief, ergonomic…">
        <button type="button" class="kw-none-btn" onclick="toggleNone('keywords')">None</button>
      </div>
    </div>

    <div class="section-divider"></div>

    <!-- 4. MODE SIMULATION / RÉEL -->
    <div class="toggle-row" style="border-top:none; padding-top:0; margin-top:0">
      ...
      <div class="toggle-label">
        <span id="mode-title">Mode simulation (Dry Run)</span>
        <span id="dry-desc">Données simulées — aucun appel API réel</span>
      </div>
    </div>

    <!-- MODE ANALYSE PRODUIT (toggle secondaire) -->
    <div class="analyze-mode-wrap" style="margin-top:16px">
      ...
      <div id="analyze-fields" style="display:none; margin-top:16px">
        <div class="field">
          ...
          <div id="url-inputs">
            <div class="url-input-row">
              ...
              <button type="button" class="url-add-btn" onclick="addUrlInput()">+</button>
            </div>
          </div>
        ...
        <button type="button" class="btn-run" id="analyze-btn" onclick="runAnalyze()" style="margin-top:12px">
          🔍 ANALYSER CE PRODUIT
        </button>
      </div>
    </div>

    <!-- 5. BOUTONS LANCER / PAUSE / STOP -->
    <div class="run-buttons-row" id="run-buttons-row">
      <button type="button" class="btn-run" id="run-btn" onclick="runSession()">
        ▶ LANCER LA RECHERCHE
      </button>
      <button type="button" class="btn-ctrl" id="pause-btn" onclick="pauseSession()" disabled title="Pause">
        ⏸
      </button>
      <button type="button" class="btn-ctrl btn-ctrl-stop" id="stop-btn" onclick="stopSession()" disabled title="Stop">
        ⏹
      </button>
    </div>
  </div>
...
<script>
  ...
  // ── Source chips ──
const SOURCE_LABELS = {
  facebook_ads: 'FB Ads', tiktok_videos: 'TikTok',
  tiktok_comments: 'TikTok Comm.', amazon: 'Amazon',
  google_trends: 'Trends', aliexpress: 'AliExpress',
...
document.querySelectorAll('.source-chip').forEach(chip => {
  ...
  function toggleDry() {
    ...
    if (title) title.textContent = dryRun ? 'Mode simulation (Dry Run)' : 'Mode réel (API Apify)';
  ...
  function getCommentKeywords() {
    // Champ supprime — les mots-cles TikTok sont dans le champ principal
    return [];
  }
  ...
  async function runSession() {
    ...
    if (!sources.length) { alert('Sélectionne au moins une source de données.'); return; }
    ...
    currentEventSource.onerror = () => { /* silent — keep-alive pings are OK */ };
  ...
  function pauseSession() {
    ...
    btn.classList.toggle('paused', sessionPaused);
    btn.textContent = sessionPaused ? '▶' : '⏸';
    btn.title = sessionPaused ? 'Reprendre' : 'Pause';
    const log = document.getElementById('log-box');
    const line = document.createElement('div');
    line.className = 'log-info';
    line.textContent = sessionPaused ? '⏸ Affichage en pause' : '▶ Reprise de l\'affichage';
    log.appendChild(line);
  ...
  function stopSession() {
    ...
    const line = document.createElement('div');
    line.className = 'log-err';
    line.textContent = '⏹ Session arrêtée par l\'utilisateur';
    log.appendChild(line);
    log.scrollTop = log.scrollHeight;
    resetRunButtons();
  }
  ...
  function toggleAnalyzeMode() {
    ...
    const runRow = document.getElementById('run-buttons-row');
    toggle.classList.toggle('on', analyzeMode);
    fields.style.display = analyzeMode ? 'block' : 'none';
    if (runRow) runRow.style.display = analyzeMode ? 'none' : 'flex';
